...
def set_gpio(service, action):      # full gpio control
    ...
    if action == 'off':
        ...
        #['pfun1'].Check()   # reference to GPIO object defined in the 'service' dictionary see below
...
def param_worker():
    ...
    while True:
            ...
            for s in Services:
                ...
                if Services[s]['state'] == 99:                                           # there was a change of service status triggered within this application
                    if ServiceStat:                                                      # service is now running
                        if (Services[s]
                                ['newstate'] == 1):                               # the new 'desired' status was expected to be up and running/1
        ...
        if version_info[0] >= 4:                                                # boot_time not available in ubuntu 1.2.1 psutil version
            ...
            uptime = datetime(1 ,1 ,1) + timedelta \
                (seconds=int(time.time()))      # find other way to calculate or leave it as a test only
        sse_parm['time']   = time.strftime("%H:%M:%S" ,time.gmtime())
        sse_parm['date']   = time.strftime("%d.%m.%Y" ,time.gmtime())
